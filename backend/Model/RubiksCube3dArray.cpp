...
struct Hash3d {
    // boost::hash_combine-style step used for each byte of the cube state.
    // Both GCC's libstdc++ and MSVC's STL use FNV-1a for std::hash<string>, but
    // that requires a heap allocation per call. This hashes the 54 bytes directly.
    ...
    size_t operator()(const RubiksCube3dArray& r1) const {
        ...
        for (int i = 0; i < 6; i++)
            for (int j = 0; j < 3; j++)
                for (int k = 0; k < 3; k++)
                    hash_combine(seed, (size_t)(unsigned char)r1.cube[i][j][k]);
        return seed;
    }
};