...
struct Hash1d {
    // boost::hash_combine-style step used for each byte of the flat 54-char state.
    // Avoids the heap allocation that std::hash<string> requires per call.
    ...
    size_t operator()(const RubiksCube1dArray& r1) const {
        size_t seed = 0;
        for (int i = 0; i < 54; i++)
            hash_combine(seed, (size_t)(unsigned char)r1.cube[i]);
        return seed;
    }
};