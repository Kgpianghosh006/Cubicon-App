...
struct HashBitboard {
    // splitmix64 finalizer: fully avalanche-mixes a 64-bit word.
    static size_t splitmix64(uint64_t x) {
        x += 0x9e3779b97f4a7c15ULL;
        x = (x ^ (x >> 30)) * 0xbf58476d1ce4e5b9ULL;
        x = (x ^ (x >> 27)) * 0x94d049bb133111ebULL;
        return (size_t)(x ^ (x >> 31));
    }
    // boost::hash_combine-style step: avoids commutative cancellation.
    ...
    size_t operator()(const RubiksCubeBitboard& r1) const {
        size_t seed = 0;
        for (int i = 0; i < 6; i++)
            hash_combine(seed, splitmix64(r1.bitboard[i]));
        return seed;
    }
};