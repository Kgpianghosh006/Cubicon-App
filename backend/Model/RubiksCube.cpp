...
vector<RubiksCube::MOVE> RubiksCube::randomShuffleCube(unsigned int times) {
    ...
    // Reproducibility is fully caller-controlled: do NOT call srand() here.
    // Filter: reject moves on the same face as the immediately preceding move.
    // Face group = move_index / 3  (L=0, R=1, U=2, D=3, F=4, B=5 — each owns 3
    // consecutive MOVE enum values: normal, prime, double).
    int lastFace = -1;
